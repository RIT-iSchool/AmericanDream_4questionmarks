...
export default function Societies() {
    const [societies, setSocieties] = useState([]); // State to store societies from DB
    const [value, setValue] = useState('1');

    useEffect(() => {
        const fetchSocieties = async () => {
            try {
                const response = await Axios.get('http://localhost:8080/societies'); // Adjust the URL as needed
                        ...
                        <Stack direction="column" spacing={4}>
                            ...
                            <Stack direction="column" spacing={4} sx={{...resultsStyles}}>
                                {societies.map((society) => {
                                    return (
                                        <Box key={society.societyID} sx={{display:'flex', flexDirection:'row', alignItems:'center', gap:'16px'}}>
                                            <div style={{backgroundColor:constStyles.primarycontainer, color:constStyles.onprimarycontainer, padding:'8px 12px', borderRadius:'50%'}}>
                                                {society.societyName[0]}
                                            </div>
